...
def clip_box(
    ...
    clipped_x_center = (clipped_x_min + clipped_x_max) / 2.0
    clipped_y_center = (clipped_y_min + clipped_y_max) / 2.0
...
def convert_label(
    ...
    for line_number, raw_line in enumerate(
        ...
        (
            ...
            height,
            was_clipped,
        ) = clip_box(x_center, y_center, width, height)
        ...
        if width <= 0.0 or height <= 0.0:
            raise ValueError(
                f"Box becomes empty after clipping in {source_label}, "
                f"line {line_number}"
            )
    ...
    return class_counts, boxes, polygons, duplicates, clipped
...
def main() -> None:
    ...
    if output_root.exists():
        raise FileExistsError(
            ...
            "Move or remove the incomplete output before rerunning."
        )
    ...
    for split, split_root in split_sources.items():
        ...
        for image_path in image_paths:
            ...
            counts, boxes, polygons, duplicates, clipped = convert_label(
                label_path,
        ...
        row: dict[str, object] = {
            ...
            "boxes_clipped_to_image": split_clipped,
        }
        for class_id, class_name in enumerate(TARGET_NAMES):
    ...
    print(f"Boxes clipped to image boundary: {total_clipped}")
